...
namespace rooset {
  ...
  class QueryException : public ProjectException<Document>
  {
  private:
    const string msg;
    const ExceptionCode exceptionCode;
    const string MESSAGE_TYPE = "QUERY_EXCEPTION";
  public:
    static const SchemaDocument schema;

    QueryException(ExceptionCode code, string msg) :
        ProjectException<Document>(msg),
        msg(msg),
        exceptionCode(code)
    {}

    QueryException(const Document& d) :
