...
CREATE TYPE "issue_state" AS ENUM (
        ...
        'canceled_conflicting_section_admitted',
...
CREATE TABLE "issue" (
        ...
        CONSTRAINT "valid_state" CHECK (
          (
            ...
            ("state" = 'canceled_conflicting_section_admitted'         AND "closed" NOTNULL AND "accepted" ISNULL) OR
...
CREATE TABLE "issue_section" (
  PRIMARY KEY ("issue_id", "external_reference"),  
  "issue_id"                    INT4            NOT NULL REFERENCES "issue" ("id") ON DELETE CASCADE ON UPDATE CASCADE,
  "external_reference"                          TEXT            NOT NULL );

COMMENT ON TABLE "issue_section" IS 'Issues have a set of section refs which are under deliberation';
...
COMMENT ON COLUMN "draft"."base_external_reference" IS 'Opaque data field to store an external reference to the base on which the external_refernce is created from';
...
CREATE VIEW "open_sections_by_area" AS
  SELECT
    "issue"."area_id",
    "issue_section"."issue_id",
    "issue_section"."external_reference" AS "section_external_reference"
  FROM "issue_section"
  JOIN "issue"
    ON "issue_section"."issue_id" = "issue"."id"
  WHERE "issue"."state" = 'discussion' 
    OR "issue"."state" = 'verification'
    OR "issue"."state" = 'voting';
