...
module.exports = (testSpec, preconditionKey) => {
  ...
  const command = events.map((e) => {
    ...
    return `/usr/bin/curl -i "http://localhost:2113/streams/aggregate-${streamId}" -H "Content-Type:application/json" -H "ES-EventType: ${eventType}" -H "ES-EventId: ${uuid()}" -d '${JSON.stringify(e)}'`
